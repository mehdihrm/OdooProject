...
class ProductTemplate(models.Model):
    ...
    show_on_hand_qty_status_button = fields.Boolean(compute='_compute_show_qty_status_button')
    show_forecasted_qty_status_button = fields.Boolean(compute='_compute_show_qty_status_button')
    # New fields --- ADDED BY MEHDI
    supply_delay = fields.Integer("Délai d'approvisionnement",help="Le délai d'approvisionnement par jour",default=0)
    security_stock = fields.Integer(help="La quantité de stock de sécurity; càd minimal du produit", compute='_compute_security_stock')
    annual_demand = fields.Integer("Demande annuelle",help="La demande annuelle n-1 du produit",default=0)
    
    
    
    @api.depends('supply_delay','annual_demand')
    def _compute_security_stock(self):
        for product in self:
            product.security_stock = int(product.annual_demand * product.supply_delay / 360)
