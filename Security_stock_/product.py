...
class ProductTemplate(models.Model):
    ...
    # fields added by Noura Hrida -22-04-2022-
    demande_annuelle = fields.Integer("demande annuelle de l’année",
                                            help="veuillez vous saisir demande annuelle de l’année  ...")
    delai_approvis = fields.Integer("Le Delai D'approvisnement",
                                    help="veuillez vous saisir le délais d’approvisionnement ")
    stock_security = fields.Integer("La Securite de stock", compute="_security_stock")
    stock_alert = fields.Integer("Stock d’alerte", compute="_stock_alert")
    stock_min = fields.Integer("Stock minimum ", compute="_stock_minimun")

    @api.depends('demande_annuelle', 'delai_approvis')
    def _security_stock(self):
        for record in self:
            record.stock_security = (record.demande_annuelle * record.delai_approvis / 365)

    @api.depends('stock_security')
    def _stock_alert(self):
        for record in self:
            record.stock_alert = (record.stock_security * 2 / 3)

    @api.depends('stock_security')
    def _stock_minimun(self):
        for record in self:
            record.stock_min = (record.stock_security / 2)
